...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const checkStoredAuth = async () => {
      setIsLoading(true);
      try {
        console.log('AuthProvider: Checking stored auth status...');
        ...
        if (isAuth) {
          ...
          if (currentUser) {
            ...
            console.log('AuthProvider: No user data, logging out');
            setUser(null);
            setToken(null);
        ...
        setIsLoading(false);
        console.log('AuthProvider: Finished checking auth status, isLoading = false');
      }
    };
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const token = authService.getToken();
        console.log('AuthProvider: Token after login', token);

        // Get the current user which should extract info from the JWT
        const currentUser = authService.getCurrentUser();
        console.log('AuthProvider: Current user after login', currentUser);
      ...
      setIsLoading(false);
      console.log('AuthProvider: Login process finished, isLoading = false');
    }
  };
  ...
  const signup = async (username: string, email: string, password: string) => {
    ...
    try {
      ...
      if (result.success) {
        // Do NOT automatically authenticate the user after signup
